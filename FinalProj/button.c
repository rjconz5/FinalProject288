...
uint8_t button_getButton() {
	...
	//
	// Bitwise AND:
	// (GPIO_PORTE_DATA_R & 0b0000 0100) == 0b00000000 if SW3 is pushed
	// (GPIO_PORTE_DATA_R & 0b0000 0100) == 0b00000100 if SW3 is not pushed
	//
	// Bitwise OR:
	// (GPIO_PORTE_DATA_R | 0b1111 1011) == 0b11111011 if SW3 is pushed
	// (GPIO_PORTE_DATA_R | 0b1111 1011) == 0b11111111 if SW3 is not pushed
	//
	// Other techniques (Shifting and bitwise AND)
	// ((GPIO_PORTE_DATA_R >> 2) & 1) == 0 if SW3 is pushed
	// ((GPIO_PORTE_DATA_R >> 2) & 0b00000001) == 1 if SW3 is not pushed
